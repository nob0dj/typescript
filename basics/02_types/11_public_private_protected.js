...
class FrontEndDev {
    constructor() {
        this.pros = `${FrontEndDev.skill} 전문 개발자입니다.`;
    }
}
FrontEndDev.skill = 'js';
...
class Coordinate {
    constructor() {
        this.z = 30; // 자식클래스에서만 접근 가능
    }
}
Coordinate.x = 10; // 클래스안에서만 Coordinate.x로 접근 가능
Coordinate.y = 20; // 어디서든 Coordinate.y로 접근 가능
...
class Position2d {
    static translateX(val) {
        Position2d.x += val;
    }
    static translateY(val) {
        Position2d.y += val;
    }
    static printX() {
