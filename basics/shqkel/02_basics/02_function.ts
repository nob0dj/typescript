...
// 매개인자가 없을 수도 있는 경우
const z = (x? : number) => x + 1; // x : number  | undefined 와 비슷.
...
const f2 = (n : number) : number => n.toString().length;
console.log(f2(245));
console.log(f2(9567));
...
const canMarry = (monthlyIncome : number, hasOwnHome : boolean, attractive : string) : boolean => {
  let score = 0;
  // 월소득 만원당 1점
  score += Math.floor(monthlyIncome / 10_000);
  // 집보유여부
...
console.log(canMarry(7_000_000, false, '중')); // true
console.log(canMarry(1_000_000, false, '상')); // false
